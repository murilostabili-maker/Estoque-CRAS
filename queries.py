...
def entradas_saidas_por_mes(meses=6):
    conn = get_conn()
    q = """
        SELECT strftime('%Y-%m', data) as mes, tipo, SUM(quantidade) as total
        FROM movimentos
...
def materiais_mais_utilizados(top_n=10):
    ...
    q = """
        ...
        LIMIT ?
    """
    df = pd.read_sql_query(q, conn, params=(top_n,))
    conn.close()
    return df
...
def consumo_por_item(item_id, periodicidade="Mensal"):
    ...
    q = f"""
        SELECT strftime('{formato}', data) as periodo, SUM(quantidade) as total_saida
        FROM movimentos
...
def meses_disponiveis():
    ...
    conn = get_conn()
    rows = conn.execute(
        "SELECT DISTINCT strftime('%Y-%m', data) as mes FROM movimentos ORDER BY mes DESC"
    ).fetchall()
    conn.close()
...
def resumo_movimentos_mes(ano_mes):
    ...
    q = """
        ...
        FROM movimentos
        WHERE strftime('%Y-%m', data) = ? AND tipo IN ('ENTRADA','SAIDA')
        GROUP BY tipo
    """
    df = pd.read_sql_query(q, conn, params=(ano_mes,))
    conn.close()
...
def materiais_mais_utilizados_periodo(ano_mes, top_n=10):
    ...
    q = """
        SELECT i.nome as item, SUM(m.quantidade) as total_saida
        FROM movimentos m JOIN itens i ON i.id = m.item_id
        WHERE m.tipo = 'SAIDA' AND strftime('%Y-%m', m.data) = ?
        GROUP BY i.nome
        ORDER BY total_saida DESC
        LIMIT ?
    """
    df = pd.read_sql_query(q, conn, params=(ano_mes, top_n))
    conn.close()
    return df
